# collector-multiverse

VaniaMetrics collector for Multiverse. One module = one jar, loaded by the platform if — and only if — the core is present.

## Build

```sh
./gradlew build                                # build/libs/vania-metrics-collector-multiverse-<v>.jar
...
- `gradle/libs.versions.toml` — paper-api, velocity-api, and the targeted plugins (Modrinth Maven repo), COMPILATION only.
